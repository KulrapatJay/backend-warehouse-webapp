...
const CreateSalesOrder = async (req, res) => {
    try {
        ...
        const created_by = req.user.id; // จาก JWT token
        ...
        for (const item of items) {
            ...
            if (productWarehouse.quantity < item.quantity) {
                throw new BadRequestException(
                    `สินค้า ${product.product_name} (${item.sku}) มีสต็อกไม่เพียงพอ (คงเหลือ: ${productWarehouse.quantity}, ต้องการ: ${item.quantity})`,
                    ErrorCodes.INSUFFICIENT_STOCK
                );
            }

            // เก็บข้อมูลสำหรับการสร้างออเดอร์
            productChecks.push({
                product_id: product.id,
                product_name: product.product_name,
                sku: product.sku,
                warehouse_quantity: productWarehouse.quantity,
                requested_quantity: item.quantity,
                unit_price: product.price // ใช้ราคาจากสินค้า
            });

            total_amount += parseFloat(product.price) * parseInt(item.quantity);
        }

        // สร้างออเดอร์และอัพเดตสต็อกในครั้งเดียว (Transaction)
            ...
            for (const item of productChecks) {
                ...
                await tx.inventory_movements.create({
                    data: {
                        ...
                        destination_warehouse_id: null, // null หมายความว่าขายออก
                        quantity_moved: item.requested_quantity,
                        movement_type: 'OUT',
                        movement_date: new Date(),
                        notes: `ขายสินค้า - ออเดอร์ ${newOrder.order_no}`,
                        created_by
                    }
                });
            }
...
const UpdateSalesOrder = async (req, res) => {
    try {
        ...
        if (status_id) {
            ...
            if (existingOrder.status.status_name === 'จัดส่งสำเร็จ' && status.status_name !== 'จัดส่งสำเร็จ') {
                throw new BadRequestException(
        ...
        for (const item of items) {
            ...
            const oldItem = existingOrder.items.find(oldItem => oldItem.product_id === product.id);
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            await tx.$executeRaw`UPDATE sales_orders SET updated_at = NOW() WHERE id = ${parseInt(id)}`;
...
const UpdateOrderStatus = async (req, res) => {
    try {
        ...
        // ตรวจสอบกฎการเปลี่ยนสถานะ
        if (existingOrder.status.status_name === 'จัดส่งสำเร็จ' && newStatus.status_name !== 'จัดส่งสำเร็จ') {
            throw new BadRequestException(
                "ไม่สามารถเปลี่ยนสถานะจาก 'จัดส่งสำเร็จ' เป็นสถานะอื่น",
                ErrorCodes.INVALID_STATUS_CHANGE
            );
